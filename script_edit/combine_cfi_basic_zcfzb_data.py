...
def main():
    ...
    temp_string = ''
    # 循环遍历列表
    for data_obj in cfi_basic_zcfzb_list:
        i = i+1
        # 字符串累加
        temp_string = temp_string + getObjToSql(data_obj,'cfi_basic_zcfzb_sheets')
        # 这个列表每2000行,或者到了最后一行就截断成为一个文件
        if i % 2000 == 0 or data_obj == cfi_basic_zcfzb_list[-1]:
            # 加一个整除的 part flag
            flag = i/2000
            # 写入一个大文件
